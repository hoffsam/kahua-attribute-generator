 ...
 */
function formatFragmentCollection(fragments, indentSize = 2) {
    const sections = [];
 ...
 */
function applyTokenTransformation(value, transformation) {
    if (!value)
        return value;
 ...
 */
function findTernaryOperator(expression) {
    let parenCount = 0;
    let questionPos = -1;
    let colonPos = -1;
    let inQuotes = false;
 ...
 */
function findLogicalOperator(expression, operator) {
    let parenCount = 0;
 ...
 */
function processStringInterpolation(template, cleanTokenValues, rawTokenValues) {
    let result = template;
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('kahua.generateAttributesExtension', () => handleSelection(['attributes'])), vscode.commands.registerCommand('kahua.generateAttributesSupplement', () => handleSelection(['supplements'])), vscode.commands.registerCommand('kahua.generateSnippetAttributes', () => generateSnippetForFragments(['attributes'])), vscode.commands.registerCommand('kahua.generateTemplateAttributes', () => generateTemplateForFragments(['attributes'])));
    // Register lookup generation commands
    context.subscriptions.push(vscode.commands.registerCommand('kahua.generateLookups', () => handleSelection(['lookups'])), vscode.commands.registerCommand('kahua.generateSnippetLookups', () => generateSnippetForFragments(['lookups'])), vscode.commands.registerCommand('kahua.generateTemplateLookups', () => generateTemplateForFragments(['lookups'])));
    // Register custom generation commands
    context.subscriptions.push(vscode.commands.registerCommand('kahua.generateCustom', async () => {
        const config = vscode.workspace.getConfiguration();
        const menuOptions = config.get('kahua.menuOptions') || [];
        if (menuOptions.length === 0) {
            vscode.window.showErrorMessage('No menu options configured. Please configure kahua.menuOptions in your settings.');
            return;
        }
        const pick = await vscode.window.showQuickPick(menuOptions.map(option => ({
            label: option.name,
 ...
 */
function parseTokenDefinition(tokens) {
    return tokens.split(',')
        .map(t => t.trim())
        .filter(Boolean)
        .map(tokenConfig => {
 ...
 */
function mergeTokenDefinitions(tokenDefs, referencedIds) {
    const headerTokens = [];
    const tableTokens = [];
    const tokenDefaults = {};
 ...
 */
function createFormattedTokenTable(tokenNames, tokenData, tokenDefaults, groupNumber) {
 ...
 */
function renderTemplate(template, cleanTokenValues, rawTokenValues, suppressWarnings) {
    const warnings = [];
 ...
 */
async function selectCustomFragments(placeholder) {
    const config = vscode.workspace.getConfiguration();
    const menuOptions = config.get('kahua.menuOptions') || [];
    if (menuOptions.length === 0) {
        vscode.window.showErrorMessage('No menu options configured. Please configure kahua.menuOptions in your settings.');
    ...
    return await vscode.window.showQuickPick(menuOptions.map(option => ({
        label: option.name,
        fragments: option.fragments
    })), {
 ...
 */
async function generateSnippetForFragments(fragmentIds) {
    ...
    try {
        const config = vscode.workspace.getConfiguration();
        const tokenDefinitions = config.get('kahua.tokenNameDefinitions') || [];
        const fragmentDefinitions = config.get('kahua.fragmentDefinitions') || [];
        ...
        const allTokenReferences = new Set();
        ...
        const { headerTokens, tableTokens } = mergeTokenDefinitions(tokenDefinitions, Array.from(allTokenReferences));
        ...
        if (tableTokens.length > 0) {
            const config = vscode.workspace.getConfiguration();
            const defaultTableRows = config.get('kahua.defaultSnippetTableRows') || 0;
 ...
 */
async function generateTemplateForFragments(fragmentIds) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('No active editor found');
        return;
    ...
    try {
        ...
        if (headerTokens.length > 0) {
            const headerTokenDisplays = headerTokens.map(token => token.defaultValue ? `${token.name}:${token.defaultValue}` : token.name);
            templateLines.push(`// Header tokens: ${headerTokenDisplays.join(', ')}`);
        }
        if (tableTokens.length > 0) {
            const tableTokenDisplays = tableTokens.map(token => token.defaultValue ? `${token.name}:${token.defaultValue}` : token.name);
 ...
 */
async function handleSelection(fragmentIds) {
    const editor = vscode.window.activeTextEditor;
    if (!editor) {
        vscode.window.showErrorMessage('No active editor found');
        return;
    ...
    try {
        ...
        // Merge token definitions based on references
        const { headerTokens, tableTokens, tokenDefaults } = mergeTokenDefinitions(tokenDefinitions, Array.from(allTokenReferences));
        // Validate selection
        ...
        for (let groupIndex = 0; groupIndex < groups.length; groupIndex++) {
            ...
            const headerTokenValues = {};
            const headerRawValues = {};
            let dataLines = group;
            ...
            for (const line of dataLines) {
                ...
                const rawTokenValues = { ...headerRawValues };
                const cleanTokenValues = { ...headerTokenValues };
                // Add table tokens
                for (let i = 0; i < tableTokens.length; i++) {
                    const token = tableTokens[i];
                ...
                for (const fragmentDef of selectedFragmentDefs) {
                    const { processedFragments, warnings: fragmentWarnings } = processFragmentTemplates(fragmentDef.fragments, cleanTokenValues, suppressWarnings);
        ...
        const formatXmlOutput = config.get('kahua.formatXmlOutput');
        const xmlIndentSize = config.get('kahua.xmlIndentSize') || 2;
